...
Testbed::NetworkDims Testbed::network_dims_neus() const {
	NetworkDims dims;
	dims.n_input = sizeof(NerfCoordinate) / sizeof(float); // == 3 + 1 + 3
	dims.n_output = 4;
...
/// Begin: Device common functions. Copy from testbed_nerf.cu

inline constexpr __device__ float NEUS_RENDERING_NEAR_DISTANCE() { return 0.05f; }
inline constexpr __device__ uint32_t NEUS_STEPS() { return 1024; } // finest number of steps per unit length
inline constexpr __device__ uint32_t NEUS_CASCADES() { return 5; }

inline constexpr __device__ float SQRT3() { return 1.73205080757f; }
inline constexpr __device__ float STEPSIZE() { return (SQRT3() / NEUS_STEPS()); } // for nerf raymarch
...
inline constexpr __device__ float MAX_CONE_STEPSIZE() { return STEPSIZE() * (1<<(NEUS_CASCADES()-1)) * NEUS_STEPS() / NERF_GRIDSIZE(); }
...
__device__ float warp_dt(float dt) {
	float max_stepsize = MIN_CONE_STEPSIZE() * (1<<(NEUS_CASCADES()-1));
...
/// End: Device common functions. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 347

/// Begin: Common global kernels 1. Copy from testbed_nerf.cu
...
__global__ void generate_grid_samples_neus_uniform(Eigen::Vector3i res_3d, const uint32_t step, BoundingBox render_aabb, BoundingBox train_aabb, NerfPosition* __restrict__ out) {
...
__global__ void generate_grid_samples_neus_uniform_dir(Eigen::Vector3i res_3d, const uint32_t step, BoundingBox render_aabb, BoundingBox train_aabb, Eigen::Vector3f ray_dir, NerfCoordinate* __restrict__ network_input) {
	// check grid_in for negative values -> must be negative on output
	uint32_t x = threadIdx.x + blockIdx.x * blockDim.x;
	uint32_t y = threadIdx.y + blockIdx.y * blockDim.y;
	uint32_t z = threadIdx.z + blockIdx.z * blockDim.z;
...
/// End: Common global kernels 1. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 437

/// Start: Mipmap device functions. Copy from testbed_nerf.cu

inline __device__ int mip_from_pos(const Vector3f& pos) {
	int exponent;
	float maxval = (pos - Vector3f::Constant(0.5f)).cwiseAbs().maxCoeff();
	frexpf(maxval, &exponent);
	return min(NEUS_CASCADES()-1, max(0, exponent+1));
}
...
inline __device__ int mip_from_dt(float dt, const Vector3f& pos) {
	...
	int exponent;
	frexpf(dt, &exponent);
	return min(NEUS_CASCADES()-1, max(exponent, mip));
}

/// End: Mipmap device functions. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 453

/// Start: Common global kernels 2. Copy from testbed_nerf.cu

__global__ void generate_grid_samples_neus_nonuniform(const uint32_t n_elements, default_rng_t rng, const uint32_t step, BoundingBox aabb, const float* __restrict__ grid_in, NerfPosition* __restrict__ out, uint32_t* __restrict__ indices, uint32_t n_cascades, float thresh) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
...
__global__ void ema_grid_samples_neus(const uint32_t n_elements,
	float decay,
...
__global__ void decay_sharpness_grid_neus(const uint32_t n_elements, float decay, float* __restrict__ grid) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
	grid[i] *= decay;
}
...
// Compute NeRF input payloads.
__global__ void advance_pos_neus(
...
__global__ void generate_neus_network_inputs_from_positions(const uint32_t n_elements, BoundingBox aabb, const Vector3f* __restrict__ pos, PitchedPtr<NerfCoordinate> network_input, Eigen::Vector3f light_dir) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
...
__global__ void generate_neus_network_inputs_at_current_position(const uint32_t n_elements, BoundingBox aabb, const NerfPayload* __restrict__ payloads, PitchedPtr<NerfCoordinate> network_input, Eigen::Vector3f light_dir) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
...
__global__ void generate_next_neus_network_inputs(
	const uint32_t n_elements,
	BoundingBox render_aabb,
...
// TODO: See testbed_nerf.cu __LINE__ 2087
__global__ void composite_kernel_neus(
	const uint32_t n_elements,
	...
	for (; j < actual_n_steps; ++j) {
		...
		if (render_mode == ERenderMode::Normals) {
			...
			if (show_accel>=0) {
				...
				rgb.x() = 1.f-mip*(1.f/(NEUS_CASCADES()-1));
...
/// End: Common global kernels 2. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 951

/// Start: Device sampling and indexing functions. Copy from testbed_nerf.cu
...
inline __device__ Vector2f neus_random_image_pos_training(default_rng_t& rng, const Vector2i& resolution, bool snap_to_pixel_centers, const float* __restrict__ cdf_x_cond_y, const float* __restrict__ cdf_y, const Vector2i& cdf_res, uint32_t img, float* __restrict__ pdf = nullptr) {
...
/// End: Device sampling and indexing functions. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 1057

/// Start: Common global kernels 3. Copy from testbed_nerf.cu

__global__ void generate_training_samples_neus(
	...
	Vector2f xy = neus_random_image_pos_training(rng, resolution, snap_to_pixel_centers, cdf_x_cond_y, cdf_y, cdf_res, img);
	...
	while (aabb.contains(pos = ray.o + t * ray.d) && j < NEUS_STEPS()) {
		float dt = calc_dt(t, cone_angle);
...
/// End: Common global kernels 3. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 1225

/// Start: LossAndGradient & read related. Copy from testbed_nerf.cu
...
__global__ void compute_loss_kernel_train_neus(
	...
	float xy_pdf = 1.0f;
	Vector2f xy = neus_random_image_pos_training(rng, resolution, snap_to_pixel_centers, cdf_x_cond_y, cdf_y, error_map_cdf_res, img, &xy_pdf);
	float max_level = max_level_rand_training ? (random_val(rng) * 2.0f) : 1.0f; // Multiply by 2 to ensure 50% of training is at max level
...
__global__ void compute_cam_gradient_train_neus(
	...
	rng.advance(ray_idx * N_MAX_RANDOM_SAMPLES_PER_RAY());
	float xy_pdf = 1.0f;
	Vector2f xy = neus_random_image_pos_training(rng, resolution, snap_to_pixel_centers, cdf_x_cond_y, cdf_y, error_map_res, img, &xy_pdf);
...
/// End: LossAndGradient & read related. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 1695

/// Start: Shading. Copy from testbed_nerf.cu

__global__ void shade_kernel_neus(
	const uint32_t n_elements,
	Array4f* __restrict__ rgba,
	float* __restrict__ depth,
	NerfPayload* __restrict__ payloads,
...
__global__ void compact_kernel_neus(
	const uint32_t n_elements,
...
__global__ void init_rays_with_payload_kernel_neus(
	uint32_t sample_index,
	NerfPayload* __restrict__ payloads,
	Vector2i resolution,
	Vector2f focal_length,
	...
	float t = fmaxf(aabb.ray_intersect(ray.o, ray.d).x(), NEUS_RENDERING_NEAR_DISTANCE()) + 1e-6f;
...
/// End: Shading. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 1861

/// Start: CDF related. Copy from testbed_nerf.cu
...
/// End: CDF related. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 1926

NGP_NEUS_NAMESPACE_END

using namespace ngp_neus;

/// Start: NeusTracer. Copy from testbed_nerf.cu

void Testbed::NeusTracer::init_rays_from_camera(
	uint32_t sample_index,
	uint32_t padded_output_width,
	...
	init_rays_with_payload_kernel_neus<<<blocks, threads, 0, stream>>>(
		sample_index,
		m_rays[0].payload,
		resolution,
		focal_length,
	...
	linear_kernel(advance_pos_neus, 0, stream,
...
// TODO: Change network structure.
uint32_t Testbed::NeusTracer::trace(
	ngp_neus::NeusNetwork<network_precision_t>& network,
	const BoundingBox& render_aabb,
	...
	while (i < MARCH_ITER) {
		...
		// Compact rays that did not diverge yet
		{
			...
			linear_kernel(compact_kernel_neus, 0, stream,
				n_alive,
		...
		linear_kernel(generate_next_neus_network_inputs, 0, stream,
			n_alive,
			render_aabb,
			train_aabb,
			focal_length,
		...
		linear_kernel(composite_kernel_neus, 0, stream,
			n_alive,
			n_elements,
			i,
			train_aabb,
...
void Testbed::NeusTracer::enlarge(size_t n_elements, uint32_t padded_output_width, uint32_t n_extra_dims, cudaStream_t stream) {
...
/// End: NeusTracer. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 2149

/// Begin: Testbed. Copy from testbed_nerf.cu

// TODO: Add m_neus, m_neus_network
void Testbed::render_neus(CudaRenderBuffer& render_buffer, const Vector2i& max_res, const Vector2f& focal_length, const Matrix<float, 3, 4>& camera_matrix0, const Matrix<float, 3, 4>& camera_matrix1, const Vector4f& rolling_shutter, const Vector2f& screen_center, cudaStream_t stream) {
	...
	bool render_opencv_camera_distortion = m_neus.render_with_camera_distortion && (!render_buffer.dlss() || m_neus.render_distortion.mode == ECameraDistortionMode::Iterative);
	bool render_grid_camera_distortion = m_neus.render_with_camera_distortion && !render_buffer.dlss();

	m_neus.tracer.init_rays_from_camera(
		render_buffer.spp(),
		m_network->padded_output_width(),
		m_neus_network->n_extra_dims(),
		...
		plane_z,
		m_dof,
		render_opencv_camera_distortion ? m_neus.render_distortion : CameraDistortion{},
		...
		m_neus.density_grid_bitfield.data(),
		m_neus.show_accel,
		m_neus.cone_angle_constant,
		render_mode,
		stream
	);
	...
	uint32_t n_hit;
	if (m_render_mode == ERenderMode::Slice) {
		n_hit = m_neus.tracer.n_rays_initialized();
	} else {
		float depth_scale = 1.0f / m_neus.training.dataset.scale;
		n_hit = m_neus.tracer.trace(
			*m_neus_network,
			m_render_aabb,
			m_aabb,
			m_neus.training.n_images_for_training,
			m_neus.training.transforms.data(),
			focal_length,
			m_neus.cone_angle_constant,
			m_neus.density_grid_bitfield.data(),
			render_mode,
			...
			m_neus.m_glow_mode,
			m_neus.light_dir.normalized(),
			stream
		);
	}
	RaysNerfSoa& rays_hit = m_render_mode == ERenderMode::Slice ? m_neus.tracer.rays_init() : m_neus.tracer.rays_hit();

	if (m_render_mode == ERenderMode::Slice) {
		// Store colors in the normal buffer
		uint32_t n_elements = next_multiple(n_hit, tcnn::batch_size_granularity);
		const uint32_t floats_per_coord = sizeof(NerfCoordinate) / sizeof(float) + m_neus_network->n_extra_dims();
		const uint32_t extra_stride = m_neus_network->n_extra_dims() * sizeof(float); // extra stride on top of base NerfCoordinate struct

		m_neus.vis_input.enlarge(n_elements * floats_per_coord);
		m_neus.vis_rgba.enlarge(n_elements);
		linear_kernel(generate_neus_network_inputs_at_current_position, 0, stream, n_hit, m_aabb, rays_hit.payload, PitchedPtr<NerfCoordinate>((NerfCoordinate*)m_neus.vis_input.data(), 1, 0, extra_stride), m_neus.light_dir.normalized());

		GPUMatrix<float> positions_matrix((float*)m_neus.vis_input.data(), floats_per_coord, n_elements);
		GPUMatrix<float> rgbsigma_matrix((float*)m_neus.vis_rgba.data(), 4, n_elements);
		...
		linear_kernel(shade_kernel_neus, 0, stream,
			n_hit,
			m_neus.vis_rgba.data(),
			nullptr,
			rays_hit.payload,
			m_render_mode,
			m_neus.training.linear_colors,
			render_buffer.frame_buffer(),
			render_buffer.depth_buffer()
		);
		return;
...
/// End: Testbed. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 2281

/// Start: Testbed::Neus::Training. Copy from testbed_nerf.cu

void Testbed::Neus::Training::set_camera_intrinsics(int frame_idx, float fx, float fy, float cx, float cy, float k1, float k2, float p1, float p2) {
...
void Testbed::Neus::Training::set_camera_extrinsics(int frame_idx, const Eigen::Matrix<float, 3, 4> &camera_to_world) {
	if (frame_idx < 0 || frame_idx >= dataset.n_images) {
		return;
	}
...
void Testbed::Neus::Training::reset_camera_extrinsics() {
...
Eigen::Matrix<float, 3, 4> Testbed::Neus::Training::get_camera_extrinsics(int frame_idx) {
	if (frame_idx < 0 || frame_idx >= dataset.n_images) {
...
void Testbed::Neus::Training::update_metadata(int first, int last) {
	if (last < 0) {
...
void Testbed::Neus::Training::update_transforms(int first, int last) {
	if (last < 0) {
		last=dataset.n_images;
	}
...
/// End: Testbed::Neus::Training. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 2390

/// Begin: Testbed initialization. Copy from testbed_nerf.cu

// TODO: Just load nerf dataset now.
void Testbed::create_empty_neus_dataset(size_t n_images, Eigen::Vector2i image_resolution, int aabb_scale, bool is_hdr) {
	m_neus.training.dataset = ngp::create_empty_nerf_dataset(n_images, image_resolution, aabb_scale, is_hdr);
	load_neus();
	m_neus.training.n_images_for_training = 0;
	m_training_data_available = true;
}

// TODO: Just load nerf dataset now.
void Testbed::load_neus() {
	if (!m_data_path.empty()) {
		...
		m_neus.training.dataset = ngp::load_nerf(json_paths, m_neus.sharpen);
	}

	m_neus.rgb_activation = m_neus.training.dataset.is_hdr ? ENerfActivation::Exponential : ENerfActivation::Logistic;

	m_neus.training.image_resolution = m_neus.training.dataset.image_resolution;
	m_neus.training.n_images_for_training = (int)m_neus.training.dataset.n_images;

	m_neus.training.update_metadata();

	m_neus.training.cam_pos_gradient.resize(m_neus.training.dataset.n_images, Vector3f::Zero());
	m_neus.training.cam_pos_gradient_gpu.resize_and_copy_from_host(m_neus.training.cam_pos_gradient);
	...
	m_neus.training.cam_rot_gradient.resize(m_neus.training.dataset.n_images, Vector3f::Zero());
	m_neus.training.cam_rot_gradient_gpu.resize_and_copy_from_host(m_neus.training.cam_rot_gradient);

	m_neus.training.cam_exposure_gradient.resize(m_neus.training.dataset.n_images, Array3f::Zero());
	m_neus.training.cam_exposure_gpu.resize_and_copy_from_host(m_neus.training.cam_exposure_gradient);
	m_neus.training.cam_exposure_gradient_gpu.resize_and_copy_from_host(m_neus.training.cam_exposure_gradient);

	m_neus.training.cam_focal_length_gradient = Vector2f::Zero();
	m_neus.training.cam_focal_length_gradient_gpu.resize_and_copy_from_host(&m_neus.training.cam_focal_length_gradient, 1);

	if (m_neus.training.dataset.rays_data.data()) {
		m_neus.training.near_distance = 0.0f;
		// m_neus.training.optimize_exposure = true;
	}
	...
	// m_neus.training.dataset.camera_distortion = {};
	...
	if (perturb_amount > 0.f) {
		for (uint32_t i = 0; i < m_neus.training.dataset.n_images; ++i) {
			Vector3f rot = random_val_3d(m_rng) * perturb_amount;
			float angle = rot.norm();
			rot /= angle;
			auto trans = random_val_3d(m_rng);
			m_neus.training.dataset.xforms[i].start.block<3,3>(0,0) = AngleAxisf(angle, rot).matrix() * m_neus.training.dataset.xforms[i].start.block<3,3>(0,0);
			m_neus.training.dataset.xforms[i].start.col(3) += trans * perturb_amount;
			m_neus.training.dataset.xforms[i].end.block<3,3>(0,0) = AngleAxisf(angle, rot).matrix() * m_neus.training.dataset.xforms[i].end.block<3,3>(0,0);
			m_neus.training.dataset.xforms[i].end.col(3) += trans * perturb_amount;
		}
	}

	m_neus.training.update_transforms();

	if (!m_neus.training.dataset.metadata.empty()) {
		m_neus.render_distortion = m_neus.training.dataset.metadata[0].camera_distortion;
		m_screen_center = Eigen::Vector2f::Constant(1.f) - m_neus.training.dataset.metadata[0].principal_point;
	}

	if (!is_pot(m_neus.training.dataset.aabb_scale)) {
		throw std::runtime_error{std::string{"NeRF dataset's `aabb_scale` must be a power of two, but is "} + std::to_string(m_neus.training.dataset.aabb_scale)};
	}

	int max_aabb_scale = 1 << (NEUS_CASCADES()-1);
	if (m_neus.training.dataset.aabb_scale > max_aabb_scale) {
		throw std::runtime_error{
			std::string{"NeRF dataset must have `aabb_scale <= "} + std::to_string(max_aabb_scale) +
			"`, but is " + std::to_string(m_neus.training.dataset.aabb_scale) +
			". You can increase this limit by factors of 2 by incrementing `NEUS_CASCADES()` and re-compiling."
		};
	}

	m_aabb = BoundingBox{Vector3f::Constant(0.5f), Vector3f::Constant(0.5f)};
	m_aabb.inflate(0.5f * std::min(1 << (NEUS_CASCADES()-1), m_neus.training.dataset.aabb_scale));
	m_raw_aabb = m_aabb;
	m_render_aabb = m_aabb;
	if (!m_neus.training.dataset.render_aabb.is_empty()) {
		m_render_aabb = m_neus.training.dataset.render_aabb.intersection(m_aabb);
	}

	m_neus.max_cascade = 0;
	while ((1 << m_neus.max_cascade) < m_neus.training.dataset.aabb_scale) {
		++m_neus.max_cascade;
	}

	// Perform fixed-size stepping in unit-cube scenes (like original NeRF) and exponential
	// stepping in larger scenes.
	m_neus.cone_angle_constant = m_neus.training.dataset.aabb_scale <= 1 ? 0.0f : (1.0f / 256.0f);

	m_up_dir = m_neus.training.dataset.up;
}

void Testbed::update_density_grid_neus(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
	const uint32_t n_elements = NERF_GRIDSIZE() * NERF_GRIDSIZE() * NERF_GRIDSIZE() * NEUS_CASCADES();

	m_neus.density_grid.enlarge(n_elements);

	const uint32_t n_density_grid_samples = n_uniform_density_grid_samples + n_nonuniform_density_grid_samples;

	const uint32_t padded_output_width = m_neus_network->padded_density_output_width();
	...
	if (m_training_step == 0 || m_neus.training.n_images_for_training != m_neus.training.n_images_for_training_prev) {
		m_neus.training.n_images_for_training_prev = m_neus.training.n_images_for_training;
		if (m_training_step == 0) {
			m_neus.density_grid_ema_step = 0;
		}
		// Only cull away empty regions where no camera is looking when the cameras are actually meaningful.
		if (!m_neus.training.dataset.rays_data.data()) {
			linear_kernel(mark_untrained_density_grid, 0, stream, n_elements, m_neus.density_grid.data(),
				m_neus.training.n_images_for_training,
				m_neus.training.metadata_gpu.data(),
				m_neus.training.transforms_gpu.data(),
				m_neus.training.image_resolution,
				m_training_step == 0
			);
		} else {
			CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.density_grid.data(), 0, sizeof(float)*n_elements, stream));
		}
	}
	...
	for (uint32_t i = 0; i < n_steps; ++i) {
		...
		linear_kernel(generate_grid_samples_neus_nonuniform, 0, stream,
			n_uniform_density_grid_samples,
			m_rng,
			m_neus.density_grid_ema_step,
			m_aabb,
			m_neus.density_grid.data(),
	...
	update_density_grid_mean_and_bitfield_neus(stream);
}

void Testbed::update_density_grid_mean_and_bitfield_neus(cudaStream_t stream) {
	const uint32_t n_elements = NERF_GRIDSIZE() * NERF_GRIDSIZE() * NERF_GRIDSIZE();

	size_t size_including_mips = grid_mip_offset(NEUS_CASCADES())/8;
	m_neus.density_grid_bitfield.enlarge(size_including_mips);
	m_neus.density_grid_mean.enlarge(reduce_sum_workspace_size(n_elements));

	CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.density_grid_mean.data(), 0, sizeof(float), stream));
	reduce_sum(m_neus.density_grid.data(), [n_elements] __device__ (float val) { return fmaxf(val, 0.f) / (n_elements); }, m_neus.density_grid_mean.data(), n_elements, stream);

	linear_kernel(grid_to_bitfield, 0, stream, n_elements/8 * NEUS_CASCADES(), m_neus.density_grid.data(), m_neus.density_grid_bitfield.data(), m_neus.density_grid_mean.data());

	for (uint32_t level = 1; level < NEUS_CASCADES(); ++level) {
		linear_kernel(bitfield_max_pool, 0, stream, n_elements/64, m_neus.get_density_grid_bitfield_mip(level-1), m_neus.get_density_grid_bitfield_mip(level));
	}
}

/// End: Testbed initialization. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 2609

/// Begin: Testbed::Neus::Training::Counters. Copy from testbed_nerf.cu

void Testbed::Neus::Training::Counters::prepare_for_training_steps(uint32_t n_training_steps, cudaStream_t stream) {
...
float Testbed::Neus::Training::Counters::update_after_training(uint32_t target_batch_size, uint32_t n_training_steps, cudaStream_t stream) {
...
/// End: Testbed::Neus::Training::Counters. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 2647

/// Begin: Testbed main train functions. Copy from testbed_nerf.cu

void Testbed::train_neus(uint32_t target_batch_size, uint32_t n_training_steps, cudaStream_t stream) {
	if (m_neus.training.include_sharpness_in_error) {
		size_t n_cells = NERF_GRIDSIZE() * NERF_GRIDSIZE() * NERF_GRIDSIZE() * NEUS_CASCADES();
		if (m_neus.training.sharpness_grid.size() < n_cells) {
			m_neus.training.sharpness_grid.enlarge(NERF_GRIDSIZE() * NERF_GRIDSIZE() * NERF_GRIDSIZE() * NEUS_CASCADES());
			CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.sharpness_grid.data(), 0, m_neus.training.sharpness_grid.get_bytes(), stream));
		}

		if (m_training_step == 0) {
			CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.sharpness_grid.data(), 0, m_neus.training.sharpness_grid.get_bytes(), stream));
		} else {
			linear_kernel(decay_sharpness_grid_neus, 0, stream, m_neus.training.sharpness_grid.size(), 0.95f, m_neus.training.sharpness_grid.data());
		}
	}
	m_neus.training.counters_rgb.prepare_for_training_steps(n_training_steps, stream);


	if (m_neus.training.n_steps_since_cam_update == 0) {
		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.cam_pos_gradient_gpu.data(), 0, m_neus.training.cam_pos_gradient_gpu.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.cam_rot_gradient_gpu.data(), 0, m_neus.training.cam_rot_gradient_gpu.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.cam_exposure_gradient_gpu.data(), 0, m_neus.training.cam_exposure_gradient_gpu.get_bytes(), stream));
		...
		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.cam_focal_length_gradient_gpu.data(), 0, m_neus.training.cam_focal_length_gradient_gpu.get_bytes(), stream));
	}


	if (m_neus.training.n_steps_since_error_map_update == 0) {
		uint32_t n_samples_per_image = (m_neus.training.n_steps_between_error_map_updates * m_neus.training.counters_rgb.rays_per_batch) / m_neus.training.dataset.n_images;
		m_neus.training.error_map.resolution = Vector2i::Constant((int)(std::sqrt(std::sqrt((float)n_samples_per_image)) * 3.5f)).cwiseMin(m_neus.training.image_resolution);
		m_neus.training.error_map.data.resize(m_neus.training.error_map.resolution.prod() * m_neus.training.dataset.n_images);
		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.error_map.data.data(), 0, m_neus.training.error_map.data.get_bytes(), stream));
	}

	float* envmap_gradient = m_neus.training.train_envmap ? m_envmap.envmap->gradients() : nullptr;
	if (envmap_gradient) {
	...
	for (uint32_t i = 0; i < n_training_steps; ++i) {
		train_neus_step(
			target_batch_size,
			m_neus.training.counters_rgb.rays_per_batch,
			m_neus.training.counters_rgb.numsteps_counter.data()+i,
			m_neus.training.counters_rgb.numsteps_counter_compacted.data()+i,
			m_neus.training.counters_rgb.loss.data()+i*m_neus.training.counters_rgb.rays_per_batch,
			m_training_stream
		);
	...
	m_loss_scalar = m_neus.training.counters_rgb.update_after_training(target_batch_size, n_training_steps, stream);
	...
	m_neus.training.n_steps_since_error_map_update += n_training_steps;
	...
	if (accumulate_error && m_neus.training.n_steps_since_error_map_update >= m_neus.training.n_steps_between_error_map_updates) {
		m_neus.training.error_map.cdf_resolution = m_neus.training.error_map.resolution;
		m_neus.training.error_map.cdf_x_cond_y.resize(m_neus.training.error_map.cdf_resolution.prod() * m_neus.training.dataset.n_images);
		m_neus.training.error_map.cdf_y.resize(m_neus.training.error_map.cdf_resolution.y() * m_neus.training.dataset.n_images);
		m_neus.training.error_map.cdf_img.resize(m_neus.training.dataset.n_images);

		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.error_map.cdf_x_cond_y.data(), 0, m_neus.training.error_map.cdf_x_cond_y.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.error_map.cdf_y.data(), 0, m_neus.training.error_map.cdf_y.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_neus.training.error_map.cdf_img.data(), 0, m_neus.training.error_map.cdf_img.get_bytes(), stream));

		const dim3 threads = { 16, 8, 1 };
		const dim3 blocks = { div_round_up((uint32_t)m_neus.training.error_map.cdf_resolution.y(), threads.x), div_round_up((uint32_t)m_neus.training.dataset.n_images, threads.y), 1 };
		construct_cdf_2d<<<blocks, threads, 0, stream>>>(
			m_neus.training.dataset.n_images, m_neus.training.error_map.cdf_resolution.y(), m_neus.training.error_map.cdf_resolution.x(),
			m_neus.training.error_map.data.data(),
			m_neus.training.error_map.cdf_x_cond_y.data(),
			m_neus.training.error_map.cdf_y.data()
			);
		linear_kernel(construct_cdf_1d, 0, stream,
			m_neus.training.dataset.n_images,
			m_neus.training.error_map.cdf_resolution.y(),
			m_neus.training.error_map.cdf_y.data(),
			m_neus.training.error_map.cdf_img.data()
		);

		// Compute image CDF on the CPU. It's single-threaded anyway. No use parallelizing.
		m_neus.training.error_map.pmf_img_cpu.resize(m_neus.training.error_map.cdf_img.size());
		m_neus.training.error_map.cdf_img.copy_to_host(m_neus.training.error_map.pmf_img_cpu);
		std::vector<float> cdf_img_cpu = m_neus.training.error_map.pmf_img_cpu; // Copy unnormalized PDF into CDF buffer
		float cum = 0;
		...
		for (size_t i = 0; i < cdf_img_cpu.size(); ++i) {
			...
			m_neus.training.error_map.pmf_img_cpu[i] = (1.0f - MIN_PMF) * m_neus.training.error_map.pmf_img_cpu[i] * norm + MIN_PMF / (float)m_neus.training.dataset.n_images;
			cdf_img_cpu[i] = (1.0f - MIN_PMF) * cdf_img_cpu[i] * norm + MIN_PMF * (float)(i+1) / (float)m_neus.training.dataset.n_images;
		}
		m_neus.training.error_map.cdf_img.copy_from_host(cdf_img_cpu);
		...
		m_neus.training.error_map.is_cdf_valid = true;

		m_neus.training.n_steps_between_error_map_updates = (uint32_t)(m_neus.training.n_steps_between_error_map_updates * 1.5f);
	}

	// Get extrinsics gradients
	m_neus.training.n_steps_since_cam_update += n_training_steps;

	bool train_camera = m_neus.training.optimize_extrinsics || m_neus.training.optimize_distortion || m_neus.training.optimize_focal_length || m_neus.training.optimize_exposure;
	if (train_camera && m_neus.training.n_steps_since_cam_update >= m_neus.training.n_steps_between_cam_updates) {
		float per_camera_loss_scale = (float)m_neus.training.n_images_for_training / LOSS_SCALE / (float)m_neus.training.n_steps_between_cam_updates;

		if (m_neus.training.optimize_extrinsics) {
			CUDA_CHECK_THROW(cudaMemcpyAsync(m_neus.training.cam_pos_gradient.data(), m_neus.training.cam_pos_gradient_gpu.data(), m_neus.training.cam_pos_gradient_gpu.get_bytes(), cudaMemcpyDeviceToHost, stream));
			CUDA_CHECK_THROW(cudaMemcpyAsync(m_neus.training.cam_rot_gradient.data(), m_neus.training.cam_rot_gradient_gpu.data(), m_neus.training.cam_rot_gradient_gpu.get_bytes(), cudaMemcpyDeviceToHost, stream));

			CUDA_CHECK_THROW(cudaStreamSynchronize(stream));

			// Optimization step
			for (uint32_t i = 0; i < m_neus.training.n_images_for_training; ++i) {
				Vector3f pos_gradient = m_neus.training.cam_pos_gradient[i] * per_camera_loss_scale;
				Vector3f rot_gradient = m_neus.training.cam_rot_gradient[i] * per_camera_loss_scale;

				float l2_reg = m_neus.training.extrinsic_l2_reg;
				pos_gradient += m_neus.training.cam_pos_offset[i].variable() * l2_reg;
				rot_gradient += m_neus.training.cam_rot_offset[i].variable() * l2_reg;

				m_neus.training.cam_pos_offset[i].set_learning_rate(std::max(1e-3f * std::pow(0.33f, (float)(m_neus.training.cam_pos_offset[i].step() / 128)), m_optimizer->learning_rate()/1000.0f));
				m_neus.training.cam_rot_offset[i].set_learning_rate(std::max(1e-3f * std::pow(0.33f, (float)(m_neus.training.cam_rot_offset[i].step() / 128)), m_optimizer->learning_rate()/1000.0f));

				m_neus.training.cam_pos_offset[i].step(pos_gradient);
				m_neus.training.cam_rot_offset[i].step(rot_gradient);
			}

			m_neus.training.update_transforms();
		}

		if (m_neus.training.optimize_distortion) {
			linear_kernel(safe_divide, 0, stream,
				m_distortion.map->n_params(),
				m_distortion.map->gradients(),
				m_distortion.map->gradient_weights()
			);
			m_distortion.trainer->optimizer_step(stream, LOSS_SCALE*(float)m_neus.training.n_steps_between_cam_updates);
		}

		if (m_neus.training.optimize_focal_length) {
			CUDA_CHECK_THROW(cudaMemcpyAsync(m_neus.training.cam_focal_length_gradient.data(),m_neus.training.cam_focal_length_gradient_gpu.data(),m_neus.training.cam_focal_length_gradient_gpu.get_bytes(),cudaMemcpyDeviceToHost, stream));
			CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
			Vector2f focal_length_gradient = m_neus.training.cam_focal_length_gradient * per_camera_loss_scale;
			float l2_reg = m_neus.training.intrinsic_l2_reg;
			focal_length_gradient += m_neus.training.cam_focal_length_offset.variable() * l2_reg;
			m_neus.training.cam_focal_length_offset.set_learning_rate(std::max(1e-3f * std::pow(0.33f, (float)(m_neus.training.cam_focal_length_offset.step() / 128)),m_optimizer->learning_rate() / 1000.0f));
			m_neus.training.cam_focal_length_offset.step(focal_length_gradient);
			m_neus.training.update_metadata();
		}

		if (m_neus.training.optimize_exposure) {
			CUDA_CHECK_THROW(cudaMemcpyAsync(m_neus.training.cam_exposure_gradient.data(), m_neus.training.cam_exposure_gradient_gpu.data(), m_neus.training.cam_exposure_gradient_gpu.get_bytes(), cudaMemcpyDeviceToHost, stream));

			Array3f mean_exposure = Array3f::Constant(0.0f);

			// Optimization step
			for (uint32_t i = 0; i < m_neus.training.n_images_for_training; ++i) {
				Array3f gradient = m_neus.training.cam_exposure_gradient[i] * per_camera_loss_scale;

				float l2_reg = m_neus.training.exposure_l2_reg;
				gradient += m_neus.training.cam_exposure[i].variable() * l2_reg;

				m_neus.training.cam_exposure[i].set_learning_rate(m_optimizer->learning_rate());
				m_neus.training.cam_exposure[i].step(gradient);

				mean_exposure += m_neus.training.cam_exposure[i].variable();
			}

			mean_exposure /= m_neus.training.n_images_for_training;

			// Renormalize
			std::vector<Array3f> cam_exposures(m_neus.training.n_images_for_training);
			for (uint32_t i = 0; i < m_neus.training.n_images_for_training; ++i) {
				cam_exposures[i] = m_neus.training.cam_exposure[i].variable() -= mean_exposure;
			}

			CUDA_CHECK_THROW(cudaMemcpyAsync(m_neus.training.cam_exposure_gpu.data(), cam_exposures.data(), m_neus.training.cam_exposure_gpu.get_bytes(), cudaMemcpyHostToDevice, stream));
		}
...
// TODO: Change network structure.
void Testbed::train_neus_step(uint32_t target_batch_size, uint32_t n_rays_per_batch, uint32_t* counter, uint32_t* compacted_counter, float* loss, cudaStream_t stream) {
	...
	if (m_neus.training.counters_rgb.measured_batch_size_before_compaction == 0) {
		m_neus.training.counters_rgb.measured_batch_size_before_compaction = max_inference = max_samples;
	} else {
		max_inference = next_multiple(std::min(m_neus.training.counters_rgb.measured_batch_size_before_compaction, max_samples), tcnn::batch_size_granularity);
	}
	...
	if (m_training_step == 0) {
		m_neus.training.counters_rgb.n_rays_total = 0;
	}

	uint32_t n_rays_total = m_neus.training.counters_rgb.n_rays_total;
	m_neus.training.counters_rgb.n_rays_total += n_rays_per_batch;
	m_neus.training.n_rays_since_error_map_update += n_rays_per_batch;

	// If we have an envmap, prepare its gradient buffer
	float* envmap_gradient = m_neus.training.train_envmap ? m_envmap.envmap->gradients() : nullptr;

	bool sample_focal_plane_proportional_to_error = m_neus.training.error_map.is_cdf_valid && m_neus.training.sample_focal_plane_proportional_to_error;
	bool sample_image_proportional_to_error = m_neus.training.error_map.is_cdf_valid && m_neus.training.sample_image_proportional_to_error;
	bool include_sharpness_in_error = m_neus.training.include_sharpness_in_error;
	// This is low-overhead enough to warrant always being on.
	// It makes for useful visualizations of the training error.
	bool accumulate_error = true;
	...
	linear_kernel(generate_training_samples_neus, 0, stream,
		n_rays_per_batch,
		m_aabb,
		max_inference,
		n_rays_total,
		m_rng,
		m_neus.training.dataset.rays_data.data(),
		ray_counter,
		counter,
		ray_indices,
		rays,
		...
		m_neus.training.train_envmap,
		m_neus.cone_angle_constant,
		m_distortion.map->params(),
		m_distortion.resolution,
		sample_focal_plane_proportional_to_error ? m_neus.training.error_map.cdf_x_cond_y.data() : nullptr,
		sample_focal_plane_proportional_to_error ? m_neus.training.error_map.cdf_y.data() : nullptr,
		sample_image_proportional_to_error ? m_neus.training.error_map.cdf_img.data() : nullptr,
		m_neus.training.error_map.cdf_resolution,
		m_neus.training.dataset.images_data.data()
	);
	...
	linear_kernel(compute_loss_kernel_train_neus, 0, stream,
		...
		m_color_space,
		m_neus.training.random_bg_color,
		m_neus.training.linear_colors,
		m_neus.training.dataset.images_data.data(),
		m_neus.training.n_images_for_training,
		m_neus.training.image_resolution,
		...
		sample_focal_plane_proportional_to_error ? m_neus.training.error_map.cdf_y.data() : nullptr,
		sample_image_proportional_to_error ? m_neus.training.error_map.cdf_img.data() : nullptr,
		m_neus.training.error_map.resolution,
		m_neus.training.error_map.cdf_resolution,
		include_sharpness_in_error ? m_neus.training.dataset.sharpness_data.data() : nullptr,
		m_neus.training.dataset.sharpness_resolution,
		m_neus.training.sharpness_grid.data(),
		m_neus.density_grid.data(),
		m_neus.density_grid_mean.data(),
		m_neus.training.cam_exposure_gpu.data(),
		m_neus.training.optimize_exposure ? m_neus.training.cam_exposure_gradient_gpu.data() : nullptr,
		m_neus.training.near_distance
	);
	...
	if (train_camera) {
		...
		linear_kernel(compute_cam_gradient_train_neus, 0, stream,
			n_rays_per_batch,
			n_rays_total,
			m_rng,
			m_aabb,
			...
			m_neus.training.image_resolution,
			m_neus.training.snap_to_pixel_centers,
			m_neus.training.optimize_extrinsics ? m_neus.training.cam_pos_gradient_gpu.data() : nullptr,
			m_neus.training.optimize_extrinsics ? m_neus.training.cam_rot_gradient_gpu.data() : nullptr,
			m_neus.training.n_images_for_training,
			ray_indices,
			rays,
			numsteps,
			...
			m_neus.training.optimize_distortion ? m_distortion.map->gradients() : nullptr,
			m_neus.training.optimize_distortion ? m_distortion.map->gradient_weights() : nullptr,
			m_distortion.resolution,
			m_neus.training.optimize_focal_length ? m_neus.training.cam_focal_length_gradient_gpu.data() : nullptr,
			sample_focal_plane_proportional_to_error ? m_neus.training.error_map.cdf_x_cond_y.data() : nullptr,
			sample_focal_plane_proportional_to_error ? m_neus.training.error_map.cdf_y.data() : nullptr,
			sample_image_proportional_to_error ? m_neus.training.error_map.cdf_img.data() : nullptr,
			m_neus.training.error_map.cdf_resolution
...
void Testbed::training_prep_neus(uint32_t batch_size, uint32_t n_training_steps, cudaStream_t stream) {
	float alpha = std::pow(m_neus.training.density_grid_decay, n_training_steps / 16.0f);
	uint32_t n_cascades = m_neus.max_cascade+1;
	if (m_training_step < 256) {
		update_density_grid_neus(alpha, NERF_GRIDSIZE()*NERF_GRIDSIZE()*NERF_GRIDSIZE()*n_cascades, 0, stream);
	} else {
		update_density_grid_neus(alpha, NERF_GRIDSIZE()*NERF_GRIDSIZE()*NERF_GRIDSIZE()/4*n_cascades, NERF_GRIDSIZE()*NERF_GRIDSIZE()*NERF_GRIDSIZE()/4*n_cascades, stream);
	}
}

/// End: Testbed main train functions. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 3095

/// Begin: Testbed mesh functions. Copy from testbed_nerf.cu

// TODO: generate mesh by SDF.
void Testbed::optimise_mesh_step_neus(uint32_t n_steps) {
	...
	const uint32_t padded_output_width = m_neus_network->padded_density_output_width();
	const uint32_t floats_per_coord = sizeof(NerfCoordinate) / sizeof(float) + m_neus_network->n_extra_dims();
	const uint32_t extra_stride = m_neus_network->n_extra_dims() * sizeof(float);
	...
	for (uint32_t i = 0; i < n_steps; ++i) {
		linear_kernel(generate_neus_network_inputs_from_positions, 0, m_inference_stream, n_verts, m_aabb, m_mesh.verts.data(), PitchedPtr<NerfCoordinate>((NerfCoordinate*)coords.data(), 1, 0, extra_stride), m_neus.light_dir.normalized());

		// For each optimizer step, we need the density at the given pos...
		m_neus_network->density(m_inference_stream, positions_matrix, density_matrix);
		// ...as well as the input gradient w.r.t. density, which we will store in the nerf coords.
		m_neus_network->input_gradient(m_inference_stream, 3, positions_matrix, positions_matrix);
...
void Testbed::compute_mesh_vertex_colors_neus() {
	uint32_t n_verts = (uint32_t)m_mesh.verts.size();
	if (!n_verts) {
		return;
	}
	...
	if (m_testbed_mode == ETestbedMode::Nerf) {
		...
		linear_kernel(extract_srgb_with_activation, 0, m_inference_stream, n_verts * 3, 3, mlp_out.data(), (float*)m_mesh.vert_colors.data(), m_neus.rgb_activation, m_neus.training.linear_colors);
	}
}

GPUMemory<float> Testbed::get_density_on_grid_neus(Vector3i res3d, const BoundingBox& aabb) {
	...
	const uint32_t padded_output_width = nerf_mode ? m_neus_network->padded_density_output_width() : m_network->padded_output_width();
	...
	generate_grid_samples_neus_uniform<<<blocks, threads, 0, m_inference_stream>>>(res3d, m_neus.density_grid_ema_step, aabb, nerf_mode ? m_aabb : unit_cube , positions);
...
GPUMemory<Eigen::Array4f> Testbed::get_rgba_on_grid_neus(Vector3i res3d, Eigen::Vector3f ray_dir) {
	const uint32_t n_elements = (res3d.x()*res3d.y()*res3d.z());
	...
	const dim3 threads = { 16, 8, 1 };
	const dim3 blocks = { div_round_up((uint32_t)res3d.x(), threads.x), div_round_up((uint32_t)res3d.y(), threads.y), div_round_up((uint32_t)res3d.z(), threads.z) };
	generate_grid_samples_neus_uniform_dir<<<blocks, threads, 0, m_inference_stream>>>(res3d, m_neus.density_grid_ema_step, m_render_aabb, m_aabb, ray_dir, positions.data());
...
// TODO: Useless?
int Testbed::marching_cubes_neus(Vector3i res3d, const BoundingBox& aabb, float thresh) {
	...
	GPUMemory<float> density = get_density_on_grid_neus(res3d, aabb);
	...
	compute_mesh_1ring(m_mesh.verts, m_mesh.indices, m_mesh.verts_smoothed, m_mesh.vert_normals);
	compute_mesh_vertex_colors_neus();

	return (int)(m_mesh.indices.size()/3);
}

uint8_t* Testbed::Neus::get_density_grid_bitfield_mip(uint32_t mip) {
	return density_grid_bitfield.data() + grid_mip_offset(mip)/8;
}

int Testbed::find_best_training_view_neus(int default_view) {
	int bestimage = default_view;
	float bestscore = 1000.f;
	for (int i = 0; i < m_neus.training.n_images_for_training; ++i) {
		float score = (m_neus.training.dataset.xforms[i].start.col(3) - m_camera.col(3)).norm();
		score += 0.25f * (m_neus.training.dataset.xforms[i].start.col(2) - m_camera.col(2)).norm();
		if (score < bestscore) { bestscore = score; bestimage = i; }
	}
	return bestimage;
}

/// End: Testbed mesh functions. Copy from testbed_nerf.cu
/// testbed_nerf.cu __line__ 3291
